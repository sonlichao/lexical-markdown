...
describe('Markdown', () => {
    ...
    const HIGHLIGHT_TEXT_MATCH_IMPORT = Object.assign(Object.assign({}, __1.LINK), { importRegExp: /\$([^$]+?)\$/, replace: (textNode) => {
    ...
    for (const { html, md, skipImport } of IMPORT_AND_EXPORT) {
        ...
        it(`can import "${md.replace(/\n/g, '\\n')}"`, () => {
            ...
            editor.update(() => (0, __1.$convertFromMarkdownString)(md, [
                ...__1.TRANSFORMERS,
                HIGHLIGHT_TEXT_MATCH_IMPORT,
            ]), {
                discrete: true,
            });
            expect(editor.getEditorState().read(() => (0, html_1.$generateHtmlFromNodes)(editor))).toBe(html);
        });
    }
    for (const { html, md, skipExport } of IMPORT_AND_EXPORT) {
        ...
        it(`can export "${md.replace(/\n/g, '\\n')}"`, () => {
            ...
            expect(editor
                .getEditorState()
                .read(() => (0, __1.$convertToMarkdownString)(__1.TRANSFORMERS))).toBe(md);
        });
    }
});
